...
Work was tracked in [plans/README.md](plans/README.md) with detailed specs under [plans/archive/](plans/archive/). Architecture contracts live in [decisions/](decisions/) (ADRs).

| Phase | What shipped | Contract / plan |
|-------|----------------|-----------------|
| **Foundation** | `MeshModel`, `Topology`, `EdgeKey` seams, island partition, OBJ import | [ADR 0001](decisions/0001-mesh-model-and-topology.md) |
| **Step 1 — Unfold core** | Hinge-island BFS, triangle-soup 2D output (no global vertex→UV map) | [ADR 0002](decisions/0002-unfold-step-1-hinge-island.md) |
| **Step 2 — Product path** | `unfoldMesh`, `layoutIslands`, Flatten action, `UnfoldViewer2D`, split viewport | [archive/step-2-flattening.md](plans/archive/step-2-flattening.md) |
| **Step 2 stretch** | Red seam segments on the 2D blueprint | [archive/step-2-seam-overlay.md](plans/archive/step-2-seam-overlay.md) |
...
| **Step 3 — Quality** | Intra-island collision (SAT) + edge tear detection; detect-only, does not block unfold | [ADR 0003](decisions/0003-unfold-quality-detection.md), [archive/step-3-quality-detection.md](plans/archive/step-3-quality-detection.md) |
| **Step 3 stretch** | Quality overlay in 2D viewer, summary toasts, Flatten-card toggle | [archive/step-3-quality-overlay.md](plans/archive/step-3-quality-overlay.md) |
| **UI shell** | Responsive sidebar, 2D split, mobile tabs, peek-through layout | [archive/mobile-responsive-layout.md](plans/archive/mobile-responsive-layout.md) |
| **QA remediation** | Staff audit findings fixed in ordered slices (docs + engine + UI polish) | [ADR 0004](decisions/0004-tech-debt-remediation-strategy.md), [archive/qa-audit-remediation.md](plans/archive/qa-audit-remediation.md) |
...
1. **Seams are discrete topology** — `Set<EdgeKey>`, never float-based edge matching ([ADR 0001](decisions/0001-mesh-model-and-topology.md)).
2. **Unfold output is triangle soup** — the same 3D vertex index can appear at different 2D positions on different faces; required for slits, darts, and honest SVG paths ([ADR 0002](decisions/0002-unfold-step-1-hinge-island.md)).
3. **Quality is orthogonal** — collisions and tears are reported for the user; they do not silently “fix” geometry ([ADR 0003](decisions/0003-unfold-quality-detection.md)).
4. **Session stability** — seam toggles do not bump `meshLoadVersion`; flatten results stay valid until a new mesh load.
5. **Remediation over big-bang refactor** — prefer targeted Zustand selectors and memoization before Web Workers for flatten ([ADR 0004](decisions/0004-tech-debt-remediation-strategy.md)).
...
1. **Plans hub as source of truth** — active and completed specs live under [docs/plans/](plans/), not only in ephemeral Cursor plan files.
...
4. **Visual QA for geometry** — for algorithmic DRY/perf changes, unit tests were required but **not sufficient**; complex seamed models were flattened and inspected on the 2D canvas ([ADR 0004](decisions/0004-tech-debt-remediation-strategy.md) Decision 2).
...
1. Pick the next row or slice from [plans/README.md](plans/README.md) or the audit plan.
...
5. Update plan checkboxes and audit IDs; promote any new spec into `docs/plans/archive/`.
...
Regression notes for Step 2: cube with a top-face seam → two separated islands and cut lines (see manual table in [step-2-flattening archive](plans/archive/step-2-flattening.md)).
